      ...
      def self.setup_guide
        {
          ...
          steps: [
            ...
            { title: "Create a external Connected App", description: "In Setup, search for 'App Manager' in the Quick Find box. Click 'New external App'. Fill in the basic info (name, email)." },
            { title: "Enable OAuth Settings", description: "Check 'Enable OAuth Settings'. Set the Callback URL to: #{ENV.fetch('VAULT_URL')}/oauth/callback/salesforce. Select OAuth scopes: 'Manage user data via APIs (api)' and 'Perform requests at any time (refresh_token, offline_access)'." },
          ...
          tips: [
            ...
            "The callback URL in Salesforce must match exactly: #{ENV.fetch('VAULT_URL')}/oauth/callback/salesforce"
          ],
          credential_help: {
